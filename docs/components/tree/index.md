...
:::demo 节点之间带连接线的树，常用于文件目录结构展示。使用 showLine 开启，可以用 switcherIcon 修改默认图标。
```vue
<template>
  <s-tree :data="data" showLine />
</template>
<script setup>
import { ref } from 'vue'
const data = ref([
...
:::demo 通过`checkable`属性控制复选框开关，默认 false
...
:::demo 通过设置 `checkboxPlacement` 属性控制复选框的位置，默认 left
```vue
<template>
  <s-tree :data="data" checkboxPlacement="right" checkable @check="check"  />
</template>
<script setup>
import { ref } from 'vue'
const check = (
