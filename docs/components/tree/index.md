...
```vue
<template>
  <s-switch v-model="showLine" />
  <s-tree :data="data" :showLine="showLine" />
</template>
<script setup>
import { ref } from 'vue';
...
```vue
<template>
  <s-tree :data="data" checkable checkbox-placement="right"  @check="check"  />
</template>
<script setup>
import { ref } from 'vue';
const check = (
...
<template>
    ...
    </template><template #content="treeNode">
...
<template>
  <h6><p>默认行为</p></h6>
  <s-tree :data="data" blockNode draggable></s-tree>

  <h6><p>排序</p></h6>
  <s-tree :data="data" blockNode :draggable="{ dropPrev: true, dropNext: true, dropInner: true }"></s-tree>
</template>
<script lang="ts">
import { defineComponent, ref } from 'vue'
...
| <a href="#field">field</a> | 配置选项，具体卡下表 | `Object` | — |  |
